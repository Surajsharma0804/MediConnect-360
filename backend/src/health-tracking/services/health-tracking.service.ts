...
export class HealthTrackingService {
  ...
  async create(userId: string, data: any): Promise<HealthTracking> {
    try {
      ...
      this.logger.log(
        `Created ${data.trackingType} tracking ${saved.id} for user ${userId}`,
      );
      return saved;
    } catch (error) {
  ...
  private calculateFitnessStats(data: HealthTracking[]): any {
    ...
    const totalActiveMinutes = data.reduce(
  ...
  private calculateSleepStats(data: HealthTracking[]): any {
    const totalSleep = data.reduce(
      (sum, d) => sum + (d.sleepDurationMinutes || 0),
      0,
    );
  ...
  private calculateNutritionStats(data: HealthTracking[]): any {
    const totalCalories = data.reduce(
      (sum, d) => sum + (d.caloriesConsumed || 0),
      0,
    );
    const totalProtein = data.reduce(
      (sum, d) => sum + (d.proteinGrams || 0),
      0,
    );
    ...
    return {
      averageCalories:
        data.length > 0 ? Math.round(totalCalories / data.length) : 0,
      averageProtein:
        data.length > 0 ? Math.round(totalProtein / data.length) : 0,
  ...
  private calculateBPStats(data: HealthTracking[]): any {
    ...
    return {
      averageSystolic:
        systolic.length > 0
          ? Math.round(systolic.reduce((a, b) => a + b, 0) / systolic.length)
          : 0,
      averageDiastolic:
        diastolic.length > 0
          ? Math.round(diastolic.reduce((a, b) => a + b, 0) / diastolic.length)
          : 0,
  ...
  private calculateGlucoseStats(data: HealthTracking[]): any {
    ...
    const avgGlucose =
      glucose.length > 0
        ? glucose.reduce((a, b) => a + b, 0) / glucose.length
        : 0;
  ...
  private calculateAverageQuality(qualities: any[]): number {
    ...
    return (
      Math.round(
        (validQualities.reduce((a, b) => a + b, 0) / validQualities.length) *
          10,
      ) / 10
    );
  }
